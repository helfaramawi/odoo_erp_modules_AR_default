...
class TestWarroomModels(TransactionCase):
    ...
    def test_all_models_create_with_defaults(self):
        for n in range(1, 5):
            ...
            self.assertEqual(rec.state, 'draft', f'x_warroom_model_{n} state default')
            self.assertEqual(rec.x_validation_status, 'draft', f'x_warroom_model_{n} validation status default')
    ...
    def test_full_state_machine_model1(self):
        ...
        for method, expected in transitions:
            getattr(rec, method)()
            self.assertEqual(rec.state, expected, f'{method} should yield state={expected}')
    ...
    def test_wizard_create_with_all_fields(self):
        ...
        self.assertEqual(wizard.reviewer, 'QA Team')
