...
class InspectionReport(models.Model):
    ...
    def action_print_inspection(self):
        return self.env.ref('stock_addition_permit.action_report_inspection').report_action(self)

    def action_view_addition_permit(self):
        self.ensure_one()
        if not self.addition_permit_id:
            return
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'stock.addition.permit',
            'res_id': self.addition_permit_id.id,
            'view_mode': 'form',
            'target': 'current',
        }
...
class StockAdditionPermit(models.Model):
    ...
    def action_print_istimara50(self):
        return self.env.ref('stock_addition_permit.action_report_istimara50').report_action(self)
...
class PurchaseOrderAdditionPermit(models.Model):
    """Extend purchase.order: smart button to related addition permits."""
    _inherit = 'purchase.order'

    addition_permit_count = fields.Integer(
        string='أذونات الإضافة',
        compute='_compute_addition_permit_count',
    )

    def _compute_addition_permit_count(self):
        PermitModel = self.env['stock.addition.permit']
        for rec in self:
            rec.addition_permit_count = PermitModel.search_count([
