...
class Form50PrintLayer(models.Model):
    ...
    def _form50_render_fields(self):
        ...
        wide_fields = {3,4,5,6,7,8,9,10,11,52,57,59,64,65,66,67,72,73,74,75}
        out = []
        for n in sorted(positions.keys(), key=lambda x: int(x)):
            ...
            if not txt:
                continue
            font_size = 6.2 if n in small_fields else 7.2 if n in wide_fields else 6.8
