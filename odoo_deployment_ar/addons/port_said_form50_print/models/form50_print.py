...
class Form50PrintLayer(models.Model):
    ...
    # لا يجوز store=True هنا: القيمة تعتمد على بحث في dossier/dossier.attachment
    # (سجلات خارجية لا تظهر في @api.depends)، فتخزينها كان يجعلها تتجمّد على
    # "لا توجد اضبارة" حتى بعد إنشاء واحدة — لازم تُحسب من جديد في كل قراءة.
