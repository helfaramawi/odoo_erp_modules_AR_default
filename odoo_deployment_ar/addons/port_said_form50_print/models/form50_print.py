...
class Form50PrintLayer(models.Model):
    ...
    def _form50_positions(self):
        ...
        return {
            # ── قسم أ — رقم ملسل وتاريخ (يمين أعلى) ────────────────────────
            1:  (84.0,  2.5),   # رقم المسلسل
            2:  (84.0,  5.0),   # تاريخ الورود

            # ── قسم أ — بيانات المصلحة والمستفيد (يسار) ─────────────────────
            3:  (26.0,  8.5),   # مصلحة
            4:  (26.0, 10.5),   # قسم / الإدارة
            5:  (24.0, 12.5),   # المبلغ المستحق إلى (اسم المورد)
            6:  (24.0, 16.5),   # بموجب / رقم الارتباط
            7:  ( 8.0, 18.5),   # صار مراجعته (اسم المورد)
            8:  (18.0, 20.5),   # إذن صرف على / البنك
            9:  (18.0, 22.5),   # شيك على الشارج (اسم المورد)
            10: (18.0, 24.5),   # يسحب باسم (اسم المورد)
            11: ( 8.0, 27.5),   # ويرسل إليه على العنوان

            # ── جدول الفواتير (يمين — 4 صفوف) ──────────────────────────────
            12: (52.0, 13.5),   # فاتورة 1 — رقم
            13: (61.5, 13.5),   # فاتورة 1 — تاريخ
            14: (71.0, 13.5),   # فاتورة 1 — جنيه
            15: (79.5, 13.5),   # فاتورة 1 — قرش
            16: (52.0, 16.5),   # فاتورة 2 — رقم
            17: (61.5, 16.5),   # فاتورة 2 — تاريخ
            18: (71.0, 16.5),   # فاتورة 2 — جنيه
            19: (79.5, 16.5),   # فاتورة 2 — قرش
            20: (52.0, 19.5),   # فاتورة 3 — رقم
            21: (61.5, 19.5),   # فاتورة 3 — تاريخ
            22: (71.0, 19.5),   # فاتورة 3 — جنيه
            23: (79.5, 19.5),   # فاتورة 3 — قرش
            24: (52.0, 22.5),   # فاتورة 4 — رقم
            25: (61.5, 22.5),   # فاتورة 4 — تاريخ
            26: (71.0, 22.5),   # فاتورة 4 — جنيه
            27: (79.5, 22.5),   # فاتورة 4 — قرش
            28: (71.0, 26.0),   # الجملة — جنيه
            29: (79.5, 26.0),   # الجملة — قرش

            # ── قسم ب — السطر الفاصل ─────────────────────────────────────────
            30: (63.0, 30.0),   # الكاتب المنوط
            31: (28.0, 30.0),   # تقييد في سجل (ز)
            32: (88.5, 34.5),   # تاريخ الشتمذ ب

            # ── الجدول الرئيسي — الاعتماد الإداري ───────────────────────────
            33: ( 5.0, 42.0),   # عدد المرفقات
            34: (45.0, 43.5),   # بند الميزانية
            35: (51.5, 43.5),   # فصل
            36: (57.5, 43.5),   # فرع / نوع
            37: (64.0, 43.5),   # قسم / باب
            38: (71.0, 43.5),   # إجمالي — جنيه
            39: (79.5, 43.5),   # إجمالي — قرش
            40: (71.0, 48.0),   # إجمالي الأصل — جنيه
            41: (79.5, 48.0),   # إجمالي الأصل — قرش

            # ── بيانات الاستقطاعات ────────────────────────────────────────────
            42: (22.0, 57.0),   # دمغة عادية — جنيه
            43: (29.5, 57.0),   # دمغة عادية — قرش
            44: (39.0, 57.0),   # دمغة إضافية — جنيه
            45: (46.0, 57.0),   # دمغة إضافية — قرش
            46: (55.0, 57.0),   # دمغة نسبية — جنيه
            47: (62.0, 57.0),   # دمغة نسبية — قرش
            48: (22.0, 61.5),   # ضريبة الأرباح — جنيه
            49: (29.5, 61.5),   # ضريبة الأرباح — قرش

            # ── صافي القيمة والتفقيط ─────────────────────────────────────────
            50: (66.0, 65.5),   # صافي القيمة — جنيه
            51: (74.0, 65.5),   # صافي القيمة — قرش
            52: (10.0, 68.5),   # التفقيط بالكلام (نص طويل)

            # ── الإقرار والتواريخ ─────────────────────────────────────────────
            53: (35.0, 71.5),   # في سنة (السنة المالية)
            54: (88.5, 71.0),   # علامة / تاريخ (الشتمذ)

            # ── توقيعات قسم ج ────────────────────────────────────────────────
            55: ( 9.0, 74.0),   # إمضاء 1 — مراقب الحسابات
            56: ( 9.0, 76.0),   # إمضاء 2 — رئيس الحسابات
            57: (52.0, 73.0),   # حساب البنك
            58: (33.0, 73.0),   # بتاريخ
            59: ( 9.0, 78.0),   # إمضاء 3 — رئيس المصلحة
            60: (88.5, 72.5),   # تاريخ الشتمذ ج

            # ── قسم ج — سجل 55 والاعتمادات ──────────────────────────────────
            61: (49.0, 77.0),   # قيد في سجل رقم 55
            62: (40.0, 79.5),   # روجع في — سنة
            63: (55.0, 79.5),   # روجع في — تاريخ
            64: (50.0, 83.0),   # شيك — اسم المستفيد
            65: (45.0, 86.0),   # يعتمد سحب — مبلغ
            66: (72.0, 89.0),   # إذن صرف — وكيل الحسابات
            67: (28.0, 89.0),   # مدير / رئيس الحسابات
            68: (40.0, 91.5),   # في سنة (صرف)
            69: (10.0, 91.0),   # بمبلغ (رقم)

            # ── قسم د ────────────────────────────────────────────────────────
            70: (88.5, 92.5),   # تاريخ الشتمذ د
            71: (36.0, 93.5),   # رقم القيد في دفتر 224
            72: (10.0, 93.5),   # إمضاء الكاتب المنوط
            73: (10.0, 96.5),   # إمضاء موظفي الشطب
            74: (36.0, 96.5),   # رقم أمر الدفع
            75: (57.0, 96.5),   # سحب / شيك — اسم
        }
