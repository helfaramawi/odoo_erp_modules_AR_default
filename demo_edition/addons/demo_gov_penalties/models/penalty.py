...
class Penalty(models.Model):
    ...
    # ── المخالفة ────────────────────────────────────────────────────────────
    violation_type_id = fields.Many2one('demo_gov.penalty.violation_type',
    ...
    # ── الجزاء المُقترَح ─────────────────────────────────────────────────────
    penalty_type_option_id = fields.Many2one('demo_gov.penalty.type_option',
    ...
    daftar55_id = fields.Many2one('demo_gov.daftar55',
        string='قيد دفتر 55 المرتبط',
        help='للجزاءات المُحصَّلة عبر أذون صرف.')

    # ── التظلم (Appeal) ────────────────────────────────────────────────────
    appeal_ids = fields.One2many('demo_gov.penalty.appeal',
    ...
    def action_record(self):
        ...
        for rec in self:
            ...
            if not rec.sequence_number:
                ...
                seq_code = ('demo_gov.penalty.employee'
                           if rec.subject_type == 'employee'
                           else 'demo_gov.penalty.vendor')
    ...
    def action_approve(self):
        ...
        for rec in self:
            ...
            if rec.violation_type_id.requires_manager_approval and \
               not rec.env.user.has_group('demo_gov_penalties.group_penalty_manager'):
                raise UserError(_(
    ...
    def _create_execution_move(self):
        ...
        self.ensure_one()
        # تصميم مقصود: لا ننشئ قيداً هنا لأنه يعتمد على دليل حسابات الجهة الحكومية
    ...
    def action_appeal(self):
        """فتح تظلم — يُنشئ demo_gov.penalty.appeal."""
        for rec in self:
            ...
            # أنشئ سجل تظلم جديد
            appeal = rec.env['demo_gov.penalty.appeal'].create({
                'penalty_id': rec.id,
            ...
            return {
                ...
                'name': _('التظلم'),
                'res_model': 'demo_gov.penalty.appeal',
                'res_id': appeal.id,
                'view_mode': 'form',
                'target': 'current',
            }
