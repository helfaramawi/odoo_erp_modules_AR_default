...
class UATGenerateWizard(models.TransientModel):
    ...
    def _check_demo_environment(self):
        """Refuse to run outside the Demo Edition — mirrors the guard in
        scripts/demo-seed/demo-reset.sh (APP_ENV=demo + db name starting
        with demo_). This tool only ever needs to run against a demo
        database; blocking it elsewhere avoids someone accidentally
        flooding a real environment with synthetic UAT records."""
        dbname = self.env.cr.dbname
        if not dbname.startswith('demo_') or os.environ.get('APP_ENV') != 'demo':
            raise UserError(_(
                'هذه الأداة مخصصة فقط لبيئة نسخة العرض التجريبي (Demo Edition).\n'
                'يجب أن يبدأ اسم قاعدة البيانات بـ demo_ وأن يكون متغير البيئة '
                'APP_ENV=demo. اسم قاعدة البيانات الحالية: "%s".'
            ) % dbname)
