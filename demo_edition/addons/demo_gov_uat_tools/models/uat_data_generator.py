...
يولّد بيانات اختبار واقعية لبيئة UAT الجهة الحكومية التجريبية
"""
import logging
...
DEPARTMENTS = [
    'ديوان عام الجهة الحكومية',
    'مديرية الشؤون المالية',
    'إدارة المشتريات',
    'إدارة المخازن',
    'إدارة الموارد البشرية',
    'إدارة المشروعات',
    'مركز معلومات الجهة الحكومية',
    'إدارة الطرق',
    'إدارة النظافة',
    'إدارة الصيانة',
...
VENDORS = [
    ...
    'شركة الجهة الحكومية التجريبية للخدمات العامة',
    'شركة الأمل للصيانة والتشغيل',
]

PURCHASE_DESCRIPTIONS = [
    'توريد أجهزة حاسب آلي لمركز معلومات الجهة الحكومية',
...
class UATDataGenerator(models.AbstractModel):
    ...
    def _generate_budget(self):
        count = 0
        Budget = self.env['demo_gov.budget.plan']
        BudgetLine = self.env['demo_gov.budget.line']

        budget_scenarios = [
            {
                'name': f'{UAT_BATCH} - موازنة ديوان عام الجهة الحكومية {TODAY.year}',
                'department': 'ديوان عام الجهة الحكومية',
                'state': 'active',
                'lines': [
            ...
            },
            {
                ...
                'department': 'مركز معلومات الجهة الحكومية',
                'state': 'approved',
                'lines': [
    ...
    def _generate_commitments(self):
        count = 0
        Commitment = self.env['demo_gov.commitment']
        ...
        for state, desc, amount in scenarios:
            if self._already_exists('demo_gov.commitment', [('description', 'ilike', desc[:30])]):
                continue
    ...
    def _generate_procurement(self):
        count = 0
        Req = self.env['demo_gov.requisition']
        ...
        for state, desc, amount in requisition_scenarios:
            if self._already_exists('demo_gov.requisition', [('description', 'ilike', desc[:30])]):
                continue
            ...
            try:
                ...
                if 'uom_id' in self.env['demo_gov.requisition.line']._fields:
                    line_vals['uom_id'] = product.uom_id.id
                self.env['demo_gov.requisition.line'].create(line_vals)
            except Exception:
                pass
    ...
    def _generate_dossiers(self):
        count = 0
        Dossier = self.env['demo_gov.dossier']
    ...
    def _generate_disbursements(self):
        count = 0
        D55 = self.env['demo_gov.daftar55']
        ...
        # Generate Daftar224
        D224 = self.env['demo_gov.daftar224']
    ...
    def _generate_accounting(self):
        ...
        # Payment orders
        PayOrder = self.env['demo_gov.payment_order']
        po_scenarios = [
        ...
        for state, desc, amount in po_scenarios:
            try:
                with self.env.cr.savepoint():
                    if self._already_exists('demo_gov.payment_order', [('purpose', 'ilike', desc[:20])]):
                        continue
                    vals = {
    ...
    def _generate_projects(self):
        ...
        project_scenarios = [
            ...
            ('مشروع التحول الرقمي لمركز معلومات الجهة الحكومية',    3_000_000, '40'),
    ...
    def _generate_inventory(self):
        ...
        Committee = self.env.get('demo_gov.inspection.committee')
        if Committee:
            for i, desc in enumerate([
                ...
                if not self._already_exists('demo_gov.inspection.committee', [('name', '=', name)]):
                    try:
        ...
        Addition = self.env.get('demo_gov.warehouse.addition')
        if Addition:
            for desc in [
                ...
                name = f'{UAT_BATCH} - {desc}'
                if not self._already_exists('demo_gov.warehouse.addition', [('name', '=', name)]):
                    try:
    ...
    def _generate_hr(self):
        count = 0
        Advance = self.env['demo_gov.advance']
        ...
        for state, desc, amount in advance_scenarios:
            ...
            try:
                with self.env.cr.savepoint():
                    if self._already_exists('demo_gov.advance', [('name', '=', name)]):
                        continue
                    vals = {
                        'name': name,
                        'amount': amount,
        ...
        # Bank guarantees
        Guarantee = self.env['demo_gov.bank.guarantee']
        guarantee_scenarios = [
        ...
        for state, desc, amount, days in guarantee_scenarios:
            ...
            try:
                with self.env.cr.savepoint():
                    if self._already_exists('demo_gov.bank.guarantee', [('name', '=', name)]):
                        continue
                    vals = {
                        'name': name,
                        'amount': amount,
    ...
    def _generate_uat_scenarios(self):
        ...
        scenarios = [
            ...
            # ═══════════════════════════════════════════════════════════════
            ('BDG-001', 'اعتماد موازنة ديوان عام الجهة الحكومية', 'budget', 'الموازنة',
             'وجود موازنة تقديرية في مرحلة المسودة',
             '1. افتح وحدة الموازنة\n2. ابحث عن موازنة ديوان عام الجهة الحكومية\n3. اضغط "اعتماد"',
             'تنتقل الموازنة لحالة معتمدة وتظهر في لوحة القيادة',
             f'{UAT_BATCH} - موازنة ديوان عام الجهة الحكومية',
             'مدير مالي', 'pending'),
            ...
            ('BDG-007', 'لوحة قيادة الموازنة - رسم بياني', 'budget', 'الموازنة',
             ...
             'لوحة قيادة الموازنة - الجهة الحكومية التجريبية',
             'مدير مالي', 'pending'),
            ...
            ('PRJ-004', 'ربط مشروع بارتباط مالي', 'project', 'المشروعات',
             ...
             f'{UAT_BATCH} - مشروع التحول الرقمي لمركز معلومات الجهة الحكومية',
             'مدير مالي', 'pending'),
            # ═══════════════════════════════════════════════════════════════
            ...
            ('RPT-001', 'تقرير تنفيذ الموازنة', 'report', 'التقارير',
             ...
             'وحدة التقارير - الجهة الحكومية التجريبية',
             'مدير مالي', 'pending'),
    ...
    def _generate_auctions(self):
        ...
        scenarios = [
            ...
            {
                'description': f'{UAT_BATCH} - مزاد بيع عقار — أرض الجهة الحكومية التجريبية الجديدة',
                'auction_type': 'sale',
                'sale_type': 'property',
                'tender_method': 'public',
        ...
        for s in scenarios:
            ...
            vals = {
                ...
                'venue': 'قاعة الاجتماعات الرئيسية — ديوان عام الجهة الحكومية',
    ...
    def _generate_cheques(self):
        count = 0
        OutgoingPO = self.env['demo_gov.outgoing_po']
        ChequeBook = self.env['demo_gov.cheque.book']
    ...
    def _generate_penalties(self):
        count = 0
        Penalty = self.env['demo_gov.penalty']
        Employee = self.env['hr.employee']
    ...
    def _generate_fixed_assets(self):
        count = 0
        Asset = self.env['demo_gov.fixed.asset']
        Category = self.env['demo_gov.asset.category']
