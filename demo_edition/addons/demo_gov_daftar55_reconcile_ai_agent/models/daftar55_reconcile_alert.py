...
class DemoGovDaftar55AccountReconcileAlert(models.Model):
    _name = 'demo_gov.daftar55.account.reconcile.alert'
    ...
    daftar55_id = fields.Many2one('demo_gov.daftar55', string='سجل دفتر 55', required=True, index=True, ondelete='cascade', tracking=True)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = seq.next_by_code('demo_gov.daftar55.account.reconcile.alert') or _('New')
        return super().create(vals_list)

    def action_open_daftar55(self):
        self.ensure_one()
        return {'type': 'ir.actions.act_window', 'name': 'دفتر 55', 'res_model': 'demo_gov.daftar55', 'res_id': self.daftar55_id.id, 'view_mode': 'form', 'target': 'current'}
