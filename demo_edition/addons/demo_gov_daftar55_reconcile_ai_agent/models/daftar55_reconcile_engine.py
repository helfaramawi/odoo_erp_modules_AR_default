...
class DemoGovDaftar55AccountReconcileEngine(models.TransientModel):
    _name = 'demo_gov.daftar55.account.reconcile.engine'
    ...
    def cron_daily_daftar55_reconcile_scan(self):
        ...
        if date_field:
            domain.append((date_field, '>=', fields.Date.today() - timedelta(days=self._param_int('demo_gov_daftar55_reconcile.scan_days', 60))))
    ...
    def check_daftar55_record(self, record):
        if not record or record._name != 'demo_gov.daftar55':
            return 0
        ...
        if diff > self._param_float('demo_gov_daftar55_reconcile.amount_tolerance', 1):
        ...
        if rec_date:
            tolerance = self._param_int('demo_gov_daftar55_reconcile.date_tolerance_days', 7)
            bad = []
            for move in moves:
                if move.date:
    ...
    def _create_or_update_alert(self, record, moves, alert_type, amount, number, rec_date, budget_line, beneficiary, difference_amount, reason):
        Alert = self.env['demo_gov.daftar55.account.reconcile.alert'].sudo()
        vals = {
