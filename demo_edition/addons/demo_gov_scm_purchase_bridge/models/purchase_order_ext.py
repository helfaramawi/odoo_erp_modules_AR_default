...
class PurchaseOrderPS(models.Model):
    ...
    requisition_ps_id = fields.Many2one("demo_gov.requisition", string="طلب الاحتياج", readonly=True, index=True)
    commitment_id     = fields.Many2one("demo_gov.commitment", string="الارتباط / التجنيب",
    ...
    def _compute_form50_count(self):
        for rec in self:
            rec.form50_count = self.env["demo_gov.daftar55"].search_count(
    ...
    def button_confirm(self):
        ...
        for rec in self:
            if not self.env["demo_gov.daftar55"].search([("form50_ref", "like", rec.name or "")], limit=1):
    ...
    def _create_form50_draft(self):
        ...
        form50  = self.env["demo_gov.daftar55"].create({
    ...
    def action_view_form50(self):
        return {"type":"ir.actions.act_window","name":"استمارات 50","res_model":"demo_gov.daftar55",
