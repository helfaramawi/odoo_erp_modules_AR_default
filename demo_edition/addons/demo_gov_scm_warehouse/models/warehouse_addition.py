...
class WarehouseAdditionPermit(models.Model):
    ...
    """
    _name = "demo_gov.warehouse.addition"
    ...
    inspection_id     = fields.Many2one("demo_gov.inspection.committee", string="محضر لجنة الفحص", required=True)
    ...
    line_ids = fields.One2many("demo_gov.warehouse.addition.line","addition_id",string="الأصناف المُضافة")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("permit_number"):
                vals["permit_number"] = self.env["ir.sequence"].next_by_code("demo_gov.warehouse.addition") or "/"
...
class WarehouseAdditionLine(models.Model):
    _name = "demo_gov.warehouse.addition.line"
    _description = "بند إذن الإضافة"

    addition_id  = fields.Many2one("demo_gov.warehouse.addition", ondelete="cascade")
