...
class InspectionCommittee(models.Model):
    ...
    """
    _name = "demo_gov.inspection.committee"
    ...
    dossier_id        = fields.Many2one("demo_gov.dossier", string="الاضبارة")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("committee_number"):
                vals["committee_number"] = self.env["ir.sequence"].next_by_code("demo_gov.inspection") or "/"
    ...
    def action_finalize_and_attach_to_dossier(self):
        ...
        if self.dossier_id:
            ...
            self.env["demo_gov.dossier.attachment"].create({
    ...
    def action_print_inspection(self):
        return self.env.ref('demo_gov_scm_warehouse.action_report_inspection').report_action(self)

    def action_print_committee(self):
        return self.env.ref(
            'demo_gov_scm_warehouse.action_report_inspection'
        ).report_action(self)
