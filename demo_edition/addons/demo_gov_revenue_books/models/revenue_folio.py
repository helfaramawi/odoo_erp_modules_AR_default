...
class RevenueFolio(models.Model):
    ...
    book_id = fields.Many2one('demo_gov.revenue.book', string='الدفتر',
    ...
    budget_plan_id = fields.Many2one('demo_gov.budget.plan',
    ...
    previous_folio_id = fields.Many2one('demo_gov.revenue.folio',
    ...
    def _extract_budget_code(self, line, level):
        ...
        if line.analytic_distribution:
            for analytic_id in line.analytic_distribution.keys():
                ...
                bl = self.env['demo_gov.budget.line'].search([
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('folio_number'):
                book = self.env['demo_gov.revenue.book'].browse(vals['book_id'])
                if book.sequence_id:
    ...
    def _find_budget_line_by_code(self, code):
        """يبحث عن demo_gov.budget.line مطابق للكود حسب مستوى التجميع."""
        self.ensure_one()
        BL = self.env['demo_gov.budget.line']
    ...
    def cron_generate_monthly_folios(self):
        ...
        Book = self.env['demo_gov.revenue.book'].search([])
        created = 0
        for book in Book:
