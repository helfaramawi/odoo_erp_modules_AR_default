...
class DemoGovBudgetForecast(models.Model):
    _name = 'demo_gov.budget.forecast'
    ...
    plan_id = fields.Many2one('demo_gov.budget.plan', string='الموازنة', required=True, index=True)
    ...
    line_ids = fields.One2many('demo_gov.budget.forecast.line', 'forecast_id', string='بنود التوقع')
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = seq.next_by_code('demo_gov.budget.forecast') or 'New'
        return super().create(vals_list)

    @api.model
    def action_generate_now(self):
        plan = self.env['demo_gov.budget.plan'].search([('state', '=', 'active')], limit=1, order='fiscal_year desc, id desc')
        if not plan:
        ...
        return {
            ...
            'res_model': 'demo_gov.budget.forecast',
            'res_id': forecast.id,
            'view_mode': 'form',
            'target': 'current',
        }
    ...
    def _cron_monthly_budget_forecast(self):
        plans = self.env['demo_gov.budget.plan'].search([('state', '=', 'active')])
        for plan in plans:
    ...
    def action_print_forecast_report(self):
        return self.env.ref('demo_gov_budget_forecast_ai_agent.action_report_budget_forecast').report_action(self)
    ...
    def _generate_lines(self):
        ...
        for forecast in self:
            ...
            for budget_line in forecast.plan_id.line_ids:
                ...
                self.env['demo_gov.budget.forecast.line'].create({
...
class DemoGovBudgetForecastLine(models.Model):
    _name = 'demo_gov.budget.forecast.line'
    _description = 'بند توقع انحرافات الموازنة'
    _order = 'risk_level, budget_code'

    forecast_id = fields.Many2one('demo_gov.budget.forecast', string='تقرير التوقع', required=True, ondelete='cascade')
    budget_line_id = fields.Many2one('demo_gov.budget.line', string='بند الموازنة')
