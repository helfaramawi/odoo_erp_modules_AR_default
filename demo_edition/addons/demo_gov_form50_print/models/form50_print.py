# -*- coding: utf-8 -*-
"""
demo_gov_form50_print — طبقة الطباعة الرسمية لاستمارة 50 ع.ح

يُوسِّع demo_gov.daftar55 بطبقة طباعة فقط.
...
class Form50InvoiceLine(models.Model):
    ...
    _name        = 'demo_gov.form50.invoice.line'
    _description = 'سطر فاتورة — استمارة 50 ع.ح'
    _order       = 'sequence, id'

    daftar55_id     = fields.Many2one('demo_gov.daftar55', ondelete='cascade', required=True, index=True)
...
class Form50PrintLayer(models.Model):
    """طبقة الطباعة الرسمية — تُوسِّع demo_gov.daftar55."""
    ...
    invoice_line_ids = fields.One2many(
        'demo_gov.form50.invoice.line', 'daftar55_id', string='بيانات الفواتير',
    )
    ...
    def _compute_attachment_readiness(self):
        for rec in self:
            ...
            dossier = self.env['demo_gov.dossier'].search(
                [('daftar55_id', '=', rec.id)], limit=1
            )
            if dossier:
                # عدّ سطور المرفقات الفعلية التي لها ملف مرفوع
                actual_cnt = self.env['demo_gov.dossier.attachment'].search_count([
    ...
    def _compute_print_readiness(self):
        for rec in self:
            ...
            if not rec.attachments_complete:
                dossier = self.env['demo_gov.dossier'].search([('daftar55_id', '=', rec.id)], limit=1)
                if not dossier:
                    issues.append('❌ لا توجد اضبارة مرتبطة — أنشئ اضبارة وأضف المرفقات')
                else:
                    real_count = self.env['demo_gov.dossier.attachment'].search_count([
                        ('dossier_id', '=', dossier.id),
                        ('attachment_id', '!=', False),
                    ])
                    if real_count == 0:
    ...
    def action_print_preview(self):
        ...
        return self.env.ref('demo_gov_form50_print.action_report_form50_preview').report_action(self)
