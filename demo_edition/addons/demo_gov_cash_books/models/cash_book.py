...
class CashBook(models.Model):
    ...
    def _compute_folio_count(self):
        Folio = self.env['demo_gov.cash.folio']
        for rec in self:
    ...
    def action_print_book(self):
        ...
        return {
            ...
            'res_model': 'demo_gov.cash.print.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {'default_book_id': self.id},
        }
        ...
        for rec in recs:
            if not rec.sequence_id:
                rec.sequence_id = self.env['ir.sequence'].create({
                    ...
                    'code': 'demo_gov.cash.book.%s' % rec.code.lower(),
                    'prefix': '78/%(year)s/',
                    'padding': 4,
                    'use_date_range': True,
