...
class CashFolio(models.Model):
    ...
    book_id = fields.Many2one('demo_gov.cash.book', string='الدفتر',
    ...
    previous_folio_id = fields.Many2one('demo_gov.cash.folio',
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('folio_number'):
                book = self.env['demo_gov.cash.book'].browse(vals['book_id'])
                if book.sequence_id:
    ...
    def cron_generate_monthly_folios(self):
        ...
        Book = self.env['demo_gov.cash.book'].search([])
        created = 0
        for book in Book:
