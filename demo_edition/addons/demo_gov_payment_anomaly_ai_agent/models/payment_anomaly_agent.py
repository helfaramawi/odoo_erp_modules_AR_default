...
class PaymentAnomalyLog(models.Model):
    _name = 'demo_gov.payment.anomaly.log'
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = seq.next_by_code('demo_gov.payment.anomaly.log') or 'New'
    ...
    def action_run_full_scan(self):
        ...
        models_to_scan = [
            'demo_gov.cheque',
            'demo_gov.payment_order',
            'demo_gov.outgoing_po',
            'demo_gov.advance',
        ]
...
class PaymentAnomalyMixin(models.AbstractModel):
    _name = 'demo_gov.payment.anomaly.mixin'
    ...
    anomaly_log_id = fields.Many2one('demo_gov.payment.anomaly.log', string='آخر تحليل شذوذ', copy=False)
    ...
    def _get_transaction_type(self):
        model = self._name
        if model == 'demo_gov.cheque':
            return 'cheque'
        if model == 'demo_gov.payment_order':
            return 'payment_order'
        if model == 'demo_gov.outgoing_po':
            return 'outgoing_po'
        if model == 'demo_gov.advance':
            return 'advance'
        return 'manual'
    ...
    def _detect_payment_anomaly(self):
        self.ensure_one()
        enabled = self._param_bool('demo_gov_payment_anomaly_ai_agent.enabled', True)
        if not enabled:
            return False
        ...
        z_threshold = self._param_float('demo_gov_payment_anomaly_ai_agent.zscore_threshold', 2.0)
        min_history = self._param_int('demo_gov_payment_anomaly_ai_agent.min_history', 5)
        duplicate_days = self._param_int('demo_gov_payment_anomaly_ai_agent.duplicate_days', 30)
        large_new_vendor_amount = self._param_float('demo_gov_payment_anomaly_ai_agent.large_new_vendor_amount', 100000.0)
        fy_end_days = self._param_int('demo_gov_payment_anomaly_ai_agent.fy_end_days', 30)
        ...
        if self._name == 'demo_gov.advance':
    ...
    def _run_payment_anomaly_detection(self, manual=False):
        ...
        existing = self.env['demo_gov.payment.anomaly.log'].search([
            ('transaction_model', '=', self._name),
            ('transaction_res_id', '=', self.id),
        ], limit=1, order='id desc')

        if manual or not existing:
            log = self.env['demo_gov.payment.anomaly.log'].create(vals)
        else:
            existing.write(vals)
            log = existing
        ...
        if vals['is_flagged']:
            ...
            block = self._param_bool('demo_gov_payment_anomaly_ai_agent.block_high_risk', False)
...
class DemoGovCheque(models.Model):
    _name = 'demo_gov.cheque'
    _inherit = ['demo_gov.cheque', 'demo_gov.payment.anomaly.mixin']
...
class DemoGovPaymentOrder(models.Model):
    _name = 'demo_gov.payment_order'
    _inherit = ['demo_gov.payment_order', 'demo_gov.payment.anomaly.mixin']
...
class DemoGovOutgoingPO(models.Model):
    _name = 'demo_gov.outgoing_po'
    _inherit = ['demo_gov.outgoing_po', 'demo_gov.payment.anomaly.mixin']
...
class DemoGovAdvance(models.Model):
    _name = 'demo_gov.advance'
    _inherit = ['demo_gov.advance', 'demo_gov.payment.anomaly.mixin']
