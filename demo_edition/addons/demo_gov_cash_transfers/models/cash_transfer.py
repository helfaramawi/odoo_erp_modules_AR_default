...
class CashTransfer(models.Model):
    ...
    source_cash_book_id = fields.Many2one('demo_gov.cash.book',
    ...
    # ── الربط بالفولية ──────────────────────────────────────────────────────
    folio_id = fields.Many2one('demo_gov.cash_transfer.folio',
    ...
    def action_prepare(self):
        ...
        for rec in self:
            ...
            if not rec.sequence_number:
                seq_code = ('demo_gov.cash_transfer.outgoing'
                           if rec.direction == 'outgoing'
                           else 'demo_gov.cash_transfer.incoming')
