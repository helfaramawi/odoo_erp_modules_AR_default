...
class TransferFolio(models.Model):
    _name = 'demo_gov.cash_transfer.folio'
    ...
    # ── العلاقة بالتحويلات ─────────────────────────────────────────────────
    transfer_ids = fields.One2many('demo_gov.cash_transfer',
    ...
    previous_folio_id = fields.Many2one('demo_gov.cash_transfer.folio',
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('folio_number'):
                seq_code = (
                    'demo_gov.cash_transfer.folio.outgoing'
                    if vals.get('direction') == 'outgoing'
                    else 'demo_gov.cash_transfer.folio.incoming'
                )
    ...
    def _attach_transfers(self):
        ...
        self.ensure_one()
        Transfer = self.env['demo_gov.cash_transfer']
