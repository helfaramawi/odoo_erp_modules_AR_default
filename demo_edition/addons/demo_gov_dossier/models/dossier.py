...
class DossierAttachment(models.Model):
    _name = 'demo_gov.dossier.attachment'
    _description = 'مرفق الاضبارة'

    dossier_id      = fields.Many2one('demo_gov.dossier', string='الاضبارة', ondelete='cascade')
...
class Dossier(models.Model):
    ...
    daftar55_id     = fields.Many2one('demo_gov.daftar55', string='مرجع دفتر 55')
    ...
    attachment_ids  = fields.One2many('demo_gov.dossier.attachment', 'dossier_id', string='المرفقات')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('dossier_number'):
                ...
                seq_code = f'demo_gov.dossier.{bl}.{yr}'
                # Use global sequence and build number manually
                seq = self.env['ir.sequence'].next_by_code('demo_gov.dossier') or '0001'
