...
class DemoGovVendorPerformanceScore(models.Model):
    _name = 'demo_gov.vendor.performance.score'
    ...
    def action_recompute(self):
        for rec in self:
            self.env['demo_gov.vendor.performance.engine'].sudo().compute_vendor_score(
                rec.partner_id,
                rec.period_date
            )
        return {
