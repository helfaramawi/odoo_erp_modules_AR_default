...
class DemoGovVendorPerformanceEngine(models.TransientModel):
    _name = 'demo_gov.vendor.performance.engine'
    ...
    def _get_min_warning_score(self):
        value = self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_vendor_performance.min_warning_score',
            '60'
        )
        try:
            return float(value)
    ...
    def _get_max_volume_reference(self):
        value = self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_vendor_performance.max_volume_reference',
            '500000'
        )
        try:
            return float(value)
    ...
    def compute_vendor_score(self, partner, period_date=None):
        ...
        Score = self.env['demo_gov.vendor.performance.score'].sudo()
    ...
    def _get_penalty_counts(self, partner, date_from, date_to):
        ...
        candidate_models = [
            'demo_gov.vendor.penalty',
            'demo_gov.vendor.penalty.alert',
            'demo_gov.penalty',
            'demo_gov.penalties',
        ]
