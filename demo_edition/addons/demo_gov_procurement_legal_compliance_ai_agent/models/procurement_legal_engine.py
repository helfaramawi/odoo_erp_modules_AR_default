...
class DemoGovProcurementLegalEngine(models.TransientModel):
    _name = 'demo_gov.procurement.legal.engine'
    ...
    def _get_blocking_enabled(self):
        val = self.env['ir.config_parameter'].sudo().get_param('demo_gov_procurement_legal.block_po_confirmation', '1')
    ...
    def cron_daily_procurement_legal_compliance_scan(self):
        count = 0
        for model_name in ['procurement.adjudication', 'procurement.committee', 'purchase.order', 'demo_gov.dossier']:
    ...
    def check_and_block_if_needed(self, record, stage='before_po'):
        ...
        if not self._get_blocking_enabled():
            return True
        compliance = self.env['demo_gov.procurement.legal.compliance'].sudo().search([('source_model', '=', record._name), ('source_res_id', '=', record.id), ('stage', '=', stage)], limit=1)
    ...
    def _official_note(self, record, method, stage, score, missing, existing, critical_missing):
        method_label = dict(self.env['demo_gov.procurement.legal.compliance']._fields['procurement_method'].selection).get(method, method)
        stage_label = dict(self.env['demo_gov.procurement.legal.compliance']._fields['stage'].selection).get(stage, stage)
        return """
    ...
    def _create_or_update_compliance(self, record, stage, method, score, legal_status, missing, existing, unknown, critical_missing, attachments):
        Compliance = self.env['demo_gov.procurement.legal.compliance'].sudo()
        vals = {
