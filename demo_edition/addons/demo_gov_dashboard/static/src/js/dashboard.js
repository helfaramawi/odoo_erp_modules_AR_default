...
class PortSaidDashboard extends Component {
    static template = xml`
<div class="o_demo_gov_dashboard">
    ...
    <div class="ps-header">
        <div class="ps-brand">
            ...
            <div>
                ...
                <p>الديوان العام للمحافظة التجريبية — سلسلة التوريد</p>
            </div>
        </div>
        <div class="ps-meta">
            <div class="ps-stat">
    ...
    async loadData() {
        try {
            const data = await this.rpc('/demo_gov/dashboard/data', {});
...
// ── Register component ───────────────────────────────────────────
registry.category('actions').add('demo_gov_dashboard', PortSaidDashboard);

// ── Global drill helper for inline onclick ────────────────────────
window._psDrillKey = (key) => {
    const comp = document.querySelector('.o_demo_gov_dashboard')?.__owl__?.component;
