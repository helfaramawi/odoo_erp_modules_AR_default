...
class ProcurementCommittee(models.Model):
    ...
    def action_print_memo_governor(self):
        return self.env.ref('procurement_committee.action_report_memo_governor').report_action(self)

    def action_print_memo_civil_defense(self):
        return self.env.ref('procurement_committee.action_report_memo_civil_defense').report_action(self)

    def action_view_members(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _('أعضاء اللجنة: %s') % self.name,
            'res_model': 'committee.member',
            'view_mode': 'tree',
            'domain': [('committee_id', '=', self.id)],
        }

    def action_print_technical_minutes(self):
        return self.env.ref('procurement_committee.action_report_technical_minutes').report_action(self)

    def action_print_opening_minutes(self):
        return self.env.ref('procurement_committee.action_report_opening_minutes').report_action(self)
