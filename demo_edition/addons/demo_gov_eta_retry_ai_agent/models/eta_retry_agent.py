...
class EtaInvoice(models.Model):
    ...
    def _eta_next_retry_time(self):
        self.ensure_one()
        base_hours = self._eta_retry_param_int('demo_gov_eta_retry_ai_agent.backoff_base_hours', 2)
    ...
    def _eta_agent_process_invoice(self, manual=False):
        ...
        max_attempts = self._eta_retry_param_int('demo_gov_eta_retry_ai_agent.max_attempts', 5)
        auto_retry = self._eta_retry_param_bool('demo_gov_eta_retry_ai_agent.auto_retry_network', True)
    ...
    def _cron_eta_retry_agent(self):
        enabled = self.env['ir.config_parameter'].sudo().get_param('demo_gov_eta_retry_ai_agent.enabled', 'True')
