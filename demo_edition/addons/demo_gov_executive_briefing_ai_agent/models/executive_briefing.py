...
class DemoGovExecutiveBriefing(models.Model):
    _name = 'demo_gov.executive.briefing'
    ...
    line_ids = fields.One2many('demo_gov.executive.briefing.line', 'briefing_id', string='بنود الملخص')
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = seq.next_by_code('demo_gov.executive.briefing') or _('New')
        return super().create(vals_list)

    def action_generate_now(self):
        briefing = self.env['demo_gov.executive.briefing.engine'].sudo().generate_daily_briefing()
        return {
...
class DemoGovExecutiveBriefingLine(models.Model):
    _name = 'demo_gov.executive.briefing.line'
    _description = 'بند في الملخص التنفيذي اليومي'
    _order = 'severity desc, amount desc, id desc'

    briefing_id = fields.Many2one('demo_gov.executive.briefing', string='الملخص التنفيذي', required=True, ondelete='cascade', index=True)
