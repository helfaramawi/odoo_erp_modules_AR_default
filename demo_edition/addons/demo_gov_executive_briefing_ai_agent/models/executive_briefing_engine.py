...
class DemoGovExecutiveBriefingEngine(models.TransientModel):
    _name = 'demo_gov.executive.briefing.engine'
    ...
    def cron_generate_and_send_daily_briefing(self):
        ...
        send_auto = self.env['ir.config_parameter'].sudo().get_param('demo_gov_executive_briefing.auto_send', '1')
    ...
    def action_generate_now(self):
        ...
        return {
            ...
            'res_model': 'demo_gov.executive.briefing',
            'res_id': briefing.id,
            'view_mode': 'form',
            'target': 'current',
        }
    ...
    def generate_daily_briefing(self):
        ...
        Briefing = self.env['demo_gov.executive.briefing'].sudo()
        ...
        for vals in lines:
            ...
            self.env['demo_gov.executive.briefing.line'].sudo().create(vals)
    ...
    def _get_budget_risks(self):
        ...
        if 'demo_gov.budget.forecast.line' in self.env:
            Model = self.env['demo_gov.budget.forecast.line'].sudo()
            domain = []
        ...
        if 'demo_gov.budget.plan' in self.env:
            Plan = self.env['demo_gov.budget.plan'].sudo()
            plans = Plan.search([('state', 'in', ['active', 'approved', 'open'])] if 'state' in Plan._fields else [], limit=5)
            for plan in plans:
                plan_lines = plan.line_ids if 'line_ids' in plan._fields else self.env['demo_gov.budget.line'].sudo().search([], limit=500)
    ...
    def _get_late_cheques(self):
        lines = []
        if 'demo_gov.cheque' not in self.env:
            return lines

        Cheque = self.env['demo_gov.cheque'].sudo()
    ...
    def _get_new_penalties(self):
        ...
        candidate_models = ['demo_gov.vendor.penalty', 'demo_gov.penalty', 'demo_gov.penalties']
        for model_name in candidate_models:
            if model_name not in self.env:
                continue
            Model = self.env[model_name].sudo()
    ...
    def _get_payment_delays(self):
        lines = []
        if 'demo_gov.payment.cycle.delay' in self.env:
            Model = self.env['demo_gov.payment.cycle.delay'].sudo()
        ...
        if 'demo_gov.daftar55' in self.env:
            Model = self.env['demo_gov.daftar55'].sudo()
            date_limit = fields.Date.today() - timedelta(days=7)
            domain = []
            if 'state' in Model._fields:
    ...
    def _get_dead_stock(self):
        lines = []
        if 'demo_gov.dead.stock.alert' not in self.env:
            return lines
        Model = self.env['demo_gov.dead.stock.alert'].sudo()
    ...
    def _write_summary_text(self, lines, counts, risk_score, risk_level):
        ...
        risk_label = dict(self.env['demo_gov.executive.briefing']._fields['risk_level'].selection).get(risk_level, risk_level)
        text = []
