...
class DemoGovArabicAIAssistant(http.Controller):
    ALLOWED_PREFIXES = ('demo_gov.', 'stock.', 'procurement.', 'l10n_eg.', 'eta.', 'c10.')
    ...
    MODEL_ALIASES = {
        ...
        'شيكات': 'demo_gov.cheque', 'شيك': 'demo_gov.cheque',
        'جزاءات': 'demo_gov.penalty', 'جزاء': 'demo_gov.penalty',
        'ارتباطات': 'demo_gov.commitment', 'ارتباط': 'demo_gov.commitment',
        'الموازنة': 'demo_gov.budget.plan', 'الموازنه': 'demo_gov.budget.plan', 'موازنة': 'demo_gov.budget.plan', 'موازنه': 'demo_gov.budget.plan',
        'سلفة': 'demo_gov.advance', 'سلفه': 'demo_gov.advance', 'سلف': 'demo_gov.advance',
        ...
        'دفتر 55': 'demo_gov.daftar55', 'دفتر 224': 'demo_gov.daftar224',
    }

    @http.route('/demo_gov/ai_assistant', type='http', auth='user', website=False)
...
<body>
<div class="wrap">
  <div class="header"><div class="brand"><div class="logo">🤖</div><div><h1>اسألني بالعربي</h1><div class="sub">Ollama JSON Fixed + Local Router — قراءة آمنة فقط</div></div></div><a class="back" href="/demo_gov/dashboard">← العودة للوحة القيادة</a></div>
  <div class="panel">
...
html+='<div class="small">Domain: '+esc(JSON.stringify(res.domain||[]))+'</div></div>';out.innerHTML=html}
    ...
    def _create_log(self, vals):
        try:
            request.env['demo_gov.ai.assistant.log'].sudo().create(vals)
        except Exception:
            pass
    ...
    def _ollama_url(self):
        return request.env['ir.config_parameter'].sudo().get_param('demo_gov_ai_assistant.ollama_url', 'http://host.docker.internal:11434')

    def _ollama_model(self):
        return request.env['ir.config_parameter'].sudo().get_param('demo_gov_ai_assistant.ollama_model', 'llama3.2:1b')
