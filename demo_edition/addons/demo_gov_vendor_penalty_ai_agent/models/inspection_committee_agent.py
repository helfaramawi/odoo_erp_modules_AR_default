...
class DemoGovInspectionCommitteePenaltyAgent(models.Model):
    _inherit = 'demo_gov.inspection.committee'

    agent_penalty_id = fields.Many2one('demo_gov.penalty', string='جزاء المورد الناتج من الوكيل', readonly=True, copy=False)
    ...
    agent_penalty_log_ids = fields.One2many(
        'demo_gov.vendor.penalty.agent.log',
        'inspection_id',
    ...
    def _agent_is_enabled(self):
        return self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_vendor_penalty_ai_agent.enabled', 'True'
        ) == 'True'
    ...
    def _agent_find_duplicate_penalty(self, vendor):
        self.ensure_one()
        Penalty = self.env['demo_gov.penalty'].sudo()
    ...
    def _agent_create_vendor_penalty(self, vendor):
        ...
        penalty = self.env['demo_gov.penalty'].sudo().create(vals)
    ...
    def _agent_get_or_create_violation_type(self):
        Model = self.env['demo_gov.penalty.violation_type'].sudo()
    ...
    def _agent_get_or_create_penalty_type_option(self):
        Model = self.env['demo_gov.penalty.type_option'].sudo()
    ...
    def _agent_get_penalty_rate(self, offense_count):
        ...
        first = float(ICP.get_param('demo_gov_vendor_penalty_ai_agent.first_rate', '0.05') or 0.05)
        second = float(ICP.get_param('demo_gov_vendor_penalty_ai_agent.second_rate', '0.10') or 0.10)
        third = float(ICP.get_param('demo_gov_vendor_penalty_ai_agent.third_rate', '0.15') or 0.15)
    ...
    def _agent_create_log(self, decision, reason, trigger='', penalty=False, calc=None, technical_details=''):
        ...
        self.env['demo_gov.vendor.penalty.agent.log'].sudo().create({
    ...
    def _agent_auto_approve_penalty_if_enabled(self, penalty):
        auto = self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_vendor_penalty_ai_agent.auto_approve', 'False'
        ) == 'True'
        if not auto:
            return
