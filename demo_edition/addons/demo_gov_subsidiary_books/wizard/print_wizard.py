...
class SubsidiaryPrintWizard(models.TransientModel):
    _name = 'demo_gov.subsidiary.print.wizard'
    _description = 'ساحر طباعة الدفاتر المساعدة'

    book_id = fields.Many2one('demo_gov.subsidiary.book',
    ...
    def action_print(self):
        self.ensure_one()
        Folio = self.env['demo_gov.subsidiary.folio']
        domain = [
        ...
        report_action_xmlid = {
            'detail':   'demo_gov_subsidiary_books.action_report_subsidiary_detail',
            'totals':   'demo_gov_subsidiary_books.action_report_subsidiary_totals',
            'personal': 'demo_gov_subsidiary_books.action_report_personal_dual',
