...
class SubsidiaryFolio(models.Model):
    _name = 'demo_gov.subsidiary.folio'
    ...
    book_id = fields.Many2one('demo_gov.subsidiary.book', string='الدفتر',
    ...
    previous_folio_id = fields.Many2one('demo_gov.subsidiary.folio',
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('folio_number'):
                book = self.env['demo_gov.subsidiary.book'].browse(vals['book_id'])
                if book.sequence_id:
    ...
    def cron_generate_monthly_folios(self):
        ...
        Book = self.env['demo_gov.subsidiary.book'].search([])
