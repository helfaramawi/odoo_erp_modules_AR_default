...
class DemoGovCommitmentAgentLog(models.Model):
    _name = 'demo_gov.commitment.agent.log'
    ...
    requisition_id = fields.Many2one('demo_gov.requisition', string='الطلب', index=True, ondelete='set null')
    commitment_id = fields.Many2one('demo_gov.commitment', string='الارتباط', index=True, ondelete='set null')
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('name', 'جديد') == 'جديد':
                vals['name'] = seq.next_by_code('demo_gov.commitment.agent.log') or 'جديد'
    ...
    def action_open_requisition(self):
        ...
        return {
            ...
            'name': _('الطلب'),
            'res_model': 'demo_gov.requisition',
