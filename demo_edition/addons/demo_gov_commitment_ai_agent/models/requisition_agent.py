...
class DemoGovRequisitionCommitmentAgent(models.Model):
    _inherit = 'demo_gov.requisition'
    ...
    def _agent_process_commitment_after_requisition_approval(self):
        ...
        enabled = params.get_param('demo_gov_commitment_ai_agent.enabled', 'True') == 'True'
        auto_limit = float(params.get_param('demo_gov_commitment_ai_agent.auto_limit', '50000') or 50000)
        ...
        require_balance = params.get_param('demo_gov_commitment_ai_agent.require_available_balance', 'True') == 'True'
        strict_penalties = params.get_param('demo_gov_commitment_ai_agent.strict_vendor_penalties', 'True') == 'True'
    ...
    def _agent_get_commitment(self):
        ...
        commitment_model = self.env['demo_gov.commitment']
    ...
    def _agent_check_vendor_penalties(self):
        ...
        summaries = []
        total_count = 0
        for model_name in ['demo_gov.penalty', 'demo_gov.penalties', 'vendor.penalty', 'res.partner.penalty']:
    ...
    def _agent_notify_approver(self, commitment, reason):
        param_user_id = self.env['ir.config_parameter'].sudo().get_param('demo_gov_commitment_ai_agent.notify_user_id')
        user = False
        if param_user_id:
            try:
    ...
    def _agent_create_log(self, commitment, decision, decision_reason, technical_details='', risk_level='medium', risk_score=0.0, vendor_has_penalties=False, penalty_summary='', auto_limit=50000.0):
        ...
        return self.env['demo_gov.commitment.agent.log'].sudo().create(vals)
