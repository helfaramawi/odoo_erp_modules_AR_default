...
====================================
يضيف لنموذج demo_gov.cheque الأساسي:
...
class ChequeInherit(models.Model):
    ...
    cheque_book_id = fields.Many2one('demo_gov.cheque.book',
    ...
    bounce_followup_id = fields.Many2one('demo_gov.cheque.bounced_followup',
        string='ملف الارتداد')

    # ── الربط بأمر الدفع المرسل ────────────────────────────────────────────
    outgoing_po_id = fields.Many2one('demo_gov.outgoing_po',
            ...
            if not rec.form56_sequence:
                if rec.direction == 'outgoing':
                    rec.form56_sequence = Seq.next_by_code(
                        'demo_gov.cheque.form56.out') or '/'
                elif rec.cheque_category == 'for_collection':
                    rec.form56_sequence = Seq.next_by_code(
                        'demo_gov.cheque.form78.collection') or '/'
        return result
    ...
    def action_return(self):
        ...
        for rec in self:
            if not rec.bounce_followup_id:
                followup = self.env['demo_gov.cheque.bounced_followup'].create({
                    'cheque_id': rec.id,
