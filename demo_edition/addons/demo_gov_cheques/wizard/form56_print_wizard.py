...
class Form56PrintWizard(models.TransientModel):
    _name = 'demo_gov.cheque.form56.print.wizard'
    ...
    def action_print(self):
        ...
        if self.register_type == 'cheques':
            Cheque = self.env['demo_gov.cheque']
            domain = [
                ('direction', '=', 'outgoing'),
            ...
            return self.env.ref(
                'demo_gov_cheques.action_report_form56_cheques'
            ).report_action(records)
        else:
            PO = self.env['demo_gov.outgoing_po']
            domain = [
                ('issue_date', '>=', self.period_from),
                ('issue_date', '<=', self.period_to),
            ]
            ...
            return self.env.ref(
                'demo_gov_cheques.action_report_form56_outgoing_po'
            ).report_action(records)
