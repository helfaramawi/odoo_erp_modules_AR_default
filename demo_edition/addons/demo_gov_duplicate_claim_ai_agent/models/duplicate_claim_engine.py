...
class DemoGovDuplicateClaimEngine(models.TransientModel):
    _name = 'demo_gov.duplicate.claim.engine'
    ...
    def cron_daily_duplicate_claim_scan(self):
        count = 0
        scan_days = self._param_int('demo_gov_duplicate_claim.scan_days', 90)
        dt_from = fields.Datetime.now() - timedelta(days=scan_days)

        attachments = self.env['ir.attachment'].sudo().search([
            ('create_date', '>=', dt_from),
            ('checksum', '!=', False),
            ('res_model', 'in', ['demo_gov.dossier', 'demo_gov.daftar55', 'account.move', 'purchase.order']),
        ])
        for att in attachments:
            count += self.scan_attachment(att)

        if 'demo_gov.daftar55' in self.env:
            records = self.env['demo_gov.daftar55'].sudo().search([])
            for rec in records:
    ...
    def scan_attachment(self, attachment):
        ...
        if not attachment.checksum:
            return 0
        if attachment.res_model not in ['demo_gov.dossier', 'demo_gov.daftar55', 'account.move', 'purchase.order']:
            return 0
        ...
        similar = self.env['ir.attachment'].sudo().search([
            ('id', '!=', attachment.id),
            ('res_model', 'in', ['demo_gov.dossier', 'demo_gov.daftar55', 'account.move', 'purchase.order']),
            ('create_date', '>=', fields.Datetime.now() - timedelta(days=self._param_int('demo_gov_duplicate_claim.scan_days', 90))),
        ], limit=200)
        ...
        for other in similar:
            ...
            if ratio >= self._param_float('demo_gov_duplicate_claim.filename_similarity_threshold', 85):
    ...
    def scan_claim_record(self, record):
        ...
        if record._name == 'demo_gov.daftar55':
    ...
    def _scan_daftar55(self, rec):
        if 'demo_gov.daftar55' not in self.env:
            return 0
        ...
        for other in candidates:
            ...
            if amount and other_amount and abs(amount - other_amount) <= self._param_float('demo_gov_duplicate_claim.amount_tolerance', 1):
                if rec_date and other_date and abs((rec_date - other_date).days) <= self._param_int('demo_gov_duplicate_claim.date_tolerance_days', 3):
    ...
    def _scan_account_move(self, move):
        ...
        for other in candidates:
            ...
            if abs(amount - other_amount) <= self._param_float('demo_gov_duplicate_claim.amount_tolerance', 1):
                if rec_date and other_date and abs((rec_date - other_date).days) <= self._param_int('demo_gov_duplicate_claim.date_tolerance_days', 3):
    ...
    def _create_or_update_alert(self, alert_type, similarity_score, source_model, source_res_id, source_display_name,
        ...
        Alert = self.env['demo_gov.duplicate.claim.alert'].sudo()
