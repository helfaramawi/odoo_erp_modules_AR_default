...
class DemoGovBudgetReallocationEngine(models.TransientModel):
    _name = 'demo_gov.budget.reallocation.engine'
    ...
    def run_reallocation_recommendations(self):
        if 'demo_gov.budget.forecast' in self.env:
            forecasts = self.env['demo_gov.budget.forecast'].sudo().search([], order='create_date desc', limit=5)
            total = 0
    ...
    def _recommend_from_budget_plans(self):
        ...
        Line = self.env['demo_gov.budget.line'].sudo()
        count = 0
        ...
        for plan in plans:
            ...
            for line in lines:
                ...
                elif expected_balance > approved * 0.20 and expected_balance > self._param_float('demo_gov_budget_reallocation.min_surplus_amount', 10000):
    ...
    def _match_surplus_to_deficit(self, budget_plan, forecast, surpluses, deficit_line, needed, source_type):
        count = 0
        max_ratio = self._param_float('demo_gov_budget_reallocation.transfer_ratio', 0.80)
        min_amount = self._param_float('demo_gov_budget_reallocation.min_recommendation_amount', 1000)
    ...
    def _create_recommendation(self, budget_plan, forecast, from_line, to_line, surplus_amount, deficit_amount,
        ...
        Rec = self.env['demo_gov.budget.reallocation.recommendation'].sudo()
    ...
    def _line_budget_line(self, forecast_line):
        for fname in ['budget_line_id', 'line_id']:
            if fname in forecast_line._fields and forecast_line[fname] and forecast_line[fname]._name == 'demo_gov.budget.line':
