...
class DemoGovBudgetReallocationRecommendation(models.Model):
    _name = 'demo_gov.budget.reallocation.recommendation'
    ...
    budget_plan_id = fields.Many2one('demo_gov.budget.plan', string='خطة الموازنة', index=True, tracking=True)

    from_budget_line_id = fields.Many2one('demo_gov.budget.line', string='من بند فائض', index=True, tracking=True)
    to_budget_line_id = fields.Many2one('demo_gov.budget.line', string='إلى بند عجز', index=True, tracking=True)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = seq.next_by_code('demo_gov.budget.reallocation.recommendation') or _('New')
    ...
    def action_open_budget_plan(self):
        ...
        return {
            ...
            'name': 'خطة الموازنة',
            'res_model': 'demo_gov.budget.plan',
