...
class InsuranceFolio(models.Model):
    _name = 'demo_gov.insurance.folio'
    ...
    # ── الحركات المرتبطة ────────────────────────────────────────────────────
    movement_ids = fields.One2many('demo_gov.insurance.movement',
    ...
    previous_folio_id = fields.Many2one('demo_gov.insurance.folio',
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('folio_number'):
                seq_code = (
                    'demo_gov.insurance.folio.final'
                    if vals.get('book_classification') == 'final'
                    else 'demo_gov.insurance.folio.provisional'
                )
    ...
    def _attach_movements(self):
        ...
        self.ensure_one()
        Movement = self.env['demo_gov.insurance.movement']
        domain = [
            ('folio_id', '=', False),
