...
يُكمِّل demo_gov.bank.guarantee ليغطي الثلاث أعمدة في الدفتر الورقي:
خطاب ضمان | نقد | شيك
"""
...
class InsuranceDeposit(models.Model):
    _name = 'demo_gov.insurance_deposit'
    ...
    # ── تفاصيل الإيداع بشيك ─────────────────────────────────────────────────
    cheque_id = fields.Many2one('demo_gov.cheque',
    ...
    # ── الفولية ──────────────────────────────────────────────────────────────
    insurance_folio_id = fields.Many2one('demo_gov.insurance.folio',
    ...
    def action_activate(self):
        ...
        for rec in self:
            ...
            if not rec.form78_sequence:
                seq_code = ('demo_gov.insurance.final'
                           if rec.book_classification == 'final'
                           else 'demo_gov.insurance.provisional')
    ...
    def _create_folio_movement(self, movement_type):
        self.ensure_one()
        self.env['demo_gov.insurance.movement'].create({
