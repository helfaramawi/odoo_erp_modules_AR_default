...
class Daftar55(models.Model):
    ...
    def _compute_real_attachment_count(self):
        dossier_model = self.env['demo_gov.dossier']
        for rec in self:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('sequence_number'):
                vals['sequence_number'] = self.env['ir.sequence'].next_by_code('demo_gov.daftar55') or '/'
    ...
    def action_post(self):
        for rec in self:
            ...
            self.env['demo_gov.daftar224'].create_dual_entry_from_daftar55(rec.id)
    ...
    def action_print_daftar55(self):
        return self.env.ref('demo_gov_daftar55.action_report_daftar55').report_action(self)
