...
class DemoGovProcurementSplittingEngine(models.TransientModel):
    _name = 'demo_gov.procurement.splitting.engine'
    _description = 'محرك كشف تجزئة المشتريات'

    @api.model
    def _get_threshold_amount(self):
        value = self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_procurement_splitting.threshold_amount',
            '50000'
        )
        try:
            return float(value)
    ...
    def _get_near_ratio(self):
        value = self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_procurement_splitting.near_threshold_ratio',
            '0.75'
        )
        try:
            return float(value)
    ...
    def _get_window_days(self):
        value = self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_procurement_splitting.window_days',
            '30'
        )
        try:
            return int(value)
    ...
    def _get_scan_days(self):
        value = self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_procurement_splitting.scan_days',
            '90'
        )
        try:
            return int(value)
    ...
    def _run_full_scan(self):
        ...
        if 'demo_gov.requisition' in self.env:
            reqs = self.env['demo_gov.requisition'].sudo().search([], limit=1000)
    ...
    def _create_generic_alert(self, source_record, supplier, total_amount, count, threshold, lower,
        ...
        Alert = self.env['demo_gov.procurement.splitting.alert'].sudo()
