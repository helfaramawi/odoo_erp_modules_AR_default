...
class DemoGovVendorDataQuality(models.Model):
    _name = 'demo_gov.vendor.data.quality'
...
class DemoGovVendorDataQualityEngine(models.TransientModel):
    _name = 'demo_gov.vendor.data.quality.engine'
    _description = 'محرك فحص جودة بيانات الموردين'

    @api.model
    def _get_min_score(self):
        value = self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_vendor_quality.min_purchase_score',
            '60'
        )
        try:
    ...
    def _get_block_purchase(self):
        value = self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_vendor_quality.block_purchase_confirmation',
            '1'
        )
