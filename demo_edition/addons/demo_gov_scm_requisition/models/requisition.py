...
class PurchaseRequisitionPS(models.Model):
    ...
    line_ids     = fields.One2many("demo_gov.requisition.line","requisition_id",string="البنود")
    ...
    commitment_id      = fields.Many2one("demo_gov.commitment", string="الارتباط المنشأ", readonly=True)
    ...
    def _compute_balance(self):
        for rec in self:
            ...
            commitments = self.env["demo_gov.commitment"].search([
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get("requisition_number"):
                vals["requisition_number"] = self.env["ir.sequence"].next_by_code("demo_gov.requisition") or "/"
    ...
    def action_approve(self):
        ...
        for rec in self:
            ...
            commitment = self.env["demo_gov.commitment"].create({
                "fiscal_year":      rec.fiscal_year,
    ...
    def action_print_requisition(self):
        return self.env.ref(
            'demo_gov_scm_requisition.action_report_requisition'
        ).report_action(self)


class PurchaseRequisitionLine(models.Model):
    _name = "demo_gov.requisition.line"
    _description = "بند طلب الاحتياج"

    requisition_id   = fields.Many2one("demo_gov.requisition", ondelete="cascade")
