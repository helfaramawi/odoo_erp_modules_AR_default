...
class DemoGovDeadStockEngine(models.TransientModel):
    _name = 'demo_gov.dead.stock.engine'
    ...
    def _get_slow_days(self):
        return int(float(self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_dead_stock.slow_days', '180'
        )))

    @api.model
    def _get_dead_days(self):
        return int(float(self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_dead_stock.dead_days', '365'
        )))

    @api.model
    def _get_high_value_threshold(self):
        return float(self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_dead_stock.high_value_threshold', '10000'
        ))

    @api.model
    def _get_overstock_months(self):
        return int(float(self.env['ir.config_parameter'].sudo().get_param(
            'demo_gov_dead_stock.overstock_months', '24'
        )))
