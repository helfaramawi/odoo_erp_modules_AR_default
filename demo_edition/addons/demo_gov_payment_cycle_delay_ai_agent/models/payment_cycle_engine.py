...
class DemoGovPaymentCycleEngine(models.TransientModel):
    _name = 'demo_gov.payment.cycle.engine'
    ...
    def _stage_sla(self, stage):
        ...
        return self._param_int('demo_gov_payment_cycle_delay.sla_%s' % stage, defaults.get(stage, 5))
    ...
    def _scan_dossiers(self):
        if 'demo_gov.dossier' not in self.env:
            return 0
        Model = self.env['demo_gov.dossier'].sudo()
    ...
    def _scan_cheques(self):
        if 'demo_gov.cheque' not in self.env:
            return 0
        Model = self.env['demo_gov.cheque'].sudo()
        records = Model.search([])
        count = 0
        for rec in records:
    ...
    def _scan_account_moves(self):
        ...
        date_from = fields.Date.today() - timedelta(days=self._param_int('demo_gov_payment_cycle_delay.scan_days', 60))
    ...
    def _has_related_cheque_or_payment(self, daftar55):
        ...
        # Cheques
        if 'demo_gov.cheque' in self.env:
            ...
            for fname in ['daftar55_id', 'demo_gov_daftar55_id']:
                if fname in Cheque._fields:
        ...
        if 'demo_gov.payment_order' in self.env:
            Pay = self.env['demo_gov.payment_order'].sudo()
            domain = []
            for fname in ['name', 'number', 'ref', 'reference', 'daftar55_number']:
    ...
    def _create_or_update_alert(self, record, stage, start_dt, due_date, delay_days, sla_days, reason):
        Alert = self.env['demo_gov.payment.cycle.delay'].sudo()
